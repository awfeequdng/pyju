...
public:
  ...
  void modifyPassConfig(MaterializationResponsibility &MR,
    ...
    outs() << "MyPlugin -- Modifying pass config for " << TT.str() << " ("
           << TT.str() << "):\n";
...
private:
  static void printBlockContent(jitlink::Block &B) {
    ...
    auto InitAddr(B.getAddress() & ~(LineWidth - 1));
    auto StartAddr = B.getAddress();
    auto EndAddr = B.getAddress() + B.getSize();
    auto *Data = reinterpret_cast<const uint8_t *>(B.getContent().data());

    for (auto CurAddr = InitAddr; CurAddr != EndAddr; ++CurAddr) {
