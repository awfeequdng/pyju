...
void pyju_gc_init_page() {
    if (GC_PAGE_SZ * block_pg_cnt < pyju_page_size) {
        block_pg_cnt = pyju_page_size / GC_PAGE_SZ;
    }
}
...
static char *pyju_gc_try_alloc_pages(int pg_cnt) PYJU_NOTSAFEPOINT {
    size_t pages_sz = GC_PAGE_SZ * pg_cnt;
    //
    if (GC_PAGE_SZ > pyju_page_size) {
        // TODO: 为什么要加上一页？？
        // 因为要按 GC_PAGE_SZ对齐，并且mmap只按pyju_page_size对齐，所以需要多分配一些内存空间
        pages_sz += GC_PAGE_SZ;
    }
    char *mem = (char*)mmap(0, pages_sz, PROT_READ | PROT_WRITE, MAP_NORESERVE|MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    if (mem == MAP_FAILED)
        return NULL;
    if (GC_PAGE_SZ > pyju_page_size) {
...
// exception is thrown.
static pyju_gc_pagemeta_t *pyju_gc_alloc_new_page(void) PYJU_NOTSAFEPOINT
{
    // try to allocate a large block of memory (or a small one)
    unsigned pg, pg_cnt = block_pg_cnt;
    char *mem = NULL;
    while (1) {
        if (__likely((mem = pyju_gc_try_alloc_pages(pg_cnt))))
            break;
        size_t min_block_pg_alloc = MIN_BLOCK_PG_ALLOC;
        if (GC_PAGE_SZ * min_block_pg_alloc < pyju_page_size)
            min_block_pg_alloc = pyju_page_size / GC_PAGE_SZ; // exact division
        if (pg_cnt >= 4 * min_block_pg_alloc) {
            ...
            pyju_printf(PYJU_STDOUT, "not impl jl_throw(pyju_memory_exception)\n");
            abort();
        }
    }
    ...
    pyju_gc_pagemeta_t *page_meta =
        (pyju_gc_pagemeta_t*)pyju_gc_perm_alloc_nolock(pg_cnt * sizeof(pyju_gc_pagemeta_t), 1,
                                                   sizeof(void*), 0);
    pg = 0;
if (page_meta) {
        for (; pg < pg_cnt; pg++) {
            ...
            if (!info.pagetable1) {
                info.pagetable1 = (pagetable1_t*)pyju_gc_perm_alloc_nolock(sizeof(pagetable1_t), 1,
                                                                         sizeof(void*), 0);
            ...
            if (!info.pagetable0) {
                info.pagetable0 = (pagetable0_t*)pyju_gc_perm_alloc_nolock(sizeof(pagetable0_t), 1,
                                                                         sizeof(void*), 0);
    ...
    if (pg < pg_cnt) {
        ...
        munmap(mem + LLT_ALIGN(GC_PAGE_SZ * pg, pyju_page_size),
               GC_PAGE_SZ * pg_cnt - LLT_ALIGN(GC_PAGE_SZ * pg, pyju_page_size));
        if (pg == 0) {
            uv_mutex_unlock(&gc_perm_lock);
...
NOINLINE pyju_gc_pagemeta_t *pyju_gc_alloc_page(void) PYJU_NOTSAFEPOINT
{
    ...
    {
        pyju_gc_pagemeta_t *meta = pyju_gc_alloc_new_page();
...
void pyju_gc_free_page(void *p) PYJU_NOTSAFEPOINT
{
    ...
    struct pyju_gc_metadata_ext info = page_metadata_ext(p);
    uint32_t msk;
    ...
    if (GC_PAGE_SZ < pyju_page_size) {
        ...
        size_t n_pages = pyju_page_size / GC_PAGE_SZ; // exact division
        decommit_size = pyju_page_size;
        void *otherp = (void*)((uintptr_t)p & ~(pyju_page_size - 1)); // round down to the nearest physical page
        p = otherp;
        while (n_pages--) {
            struct pyju_gc_metadata_ext info = page_metadata_ext(otherp);
            msk = (uint32_t)(1u << info.pagetable0_i);
