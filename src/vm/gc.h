...
// the stack.
STATIC_INLINE void *gc_pop_markdata_(PyjuGcMarkSp_t *sp, size_t size)
{
    PyjuGcMarkData_t *data = (PyjuGcMarkData_t *)(((char*)sp->data) - size);
    sp->data = data;
    return data;
}
...
STATIC_INLINE void *gc_repush_markdata_(PyjuGcMarkSp_t *sp, size_t size) PYJU_NOTSAFEPOINT
{
    PyjuGcMarkData_t *data = sp->data;
    sp->pc++;
    sp->data = (PyjuGcMarkData_t *)(((char*)sp->data) + size);
    return data;
}
...
PYJU_EXTENSION typedef struct _bigval_t {
...
//  Blocks: osize * n
//    Tag: sizeof(PyjuTaggedValue_t)
//    Data: <= osize - sizeof(PyjuTaggedValue_t)
...
typedef struct {
    pyju_gc_pagemeta_t *meta[REGION0_PG_COUNT];
...
unsigned ffs_u32(uint32_t bitvec) PYJU_NOTSAFEPOINT;
#else
...
STATIC_INLINE bigval_t *bigval_header(PyjuTaggedValue_t *o) PYJU_NOTSAFEPOINT
{
    return container_of(o, bigval_t, header);
}

// round an address inside a gcpage's data to its beginning
STATIC_INLINE char *gc_page_data(void *x) PYJU_NOTSAFEPOINT
{
    return (char*)(((uintptr_t)x >> GC_PAGE_LG2) << GC_PAGE_LG2);
}

STATIC_INLINE PyjuTaggedValue_t *page_pfl_beg(pyju_gc_pagemeta_t *p) PYJU_NOTSAFEPOINT
{
    return (PyjuTaggedValue_t*)(p->data + p->fl_begin_offset);
}

STATIC_INLINE PyjuTaggedValue_t *page_pfl_end(pyju_gc_pagemeta_t *p) PYJU_NOTSAFEPOINT
{
    return (PyjuTaggedValue_t*)(p->data + p->fl_end_offset);
}

STATIC_INLINE int gc_marked(uintptr_t bits) PYJU_NOTSAFEPOINT
{
    return (bits & GC_MARKED) != 0;
}

STATIC_INLINE int gc_old(uintptr_t bits) PYJU_NOTSAFEPOINT
{
    return (bits & GC_OLD) != 0;
}


STATIC_INLINE uintptr_t gc_set_bits(uintptr_t tag, int bits) PYJU_NOTSAFEPOINT
{
    return (tag & ~(uintptr_t)3) | bits;
}

STATIC_INLINE uintptr_t gc_ptr_tag(void *v, uintptr_t mask) PYJU_NOTSAFEPOINT
{
    return ((uintptr_t)v) & mask;
}

STATIC_INLINE void *gc_ptr_clear_tag(void *v, uintptr_t mask) PYJU_NOTSAFEPOINT
{
...
STATIC_INLINE pyju_gc_pagemeta_t *page_metadata(void *_data) PYJU_NOTSAFEPOINT
{
...
struct pyju_gc_metadata_ext {
    pagetable1_t *pagetable1;
    pagetable0_t *pagetable0;
    pyju_gc_pagemeta_t *meta;
    unsigned pagetable_i32, pagetable_i;
    unsigned pagetable1_i32, pagetable1_i;
    unsigned pagetable0_i32, pagetable0_i;
};

STATIC_INLINE struct pyju_gc_metadata_ext page_metadata_ext(void *_data) PYJU_NOTSAFEPOINT
{
    uintptr_t data = (uintptr_t)_data;
    struct pyju_gc_metadata_ext info;
    unsigned i;
    i = REGION_INDEX(data);
...
STATIC_INLINE void gc_big_object_unlink(const bigval_t *hdr) PYJU_NOTSAFEPOINT
{
    *hdr->prev = hdr->next;
    if (hdr->next) {
...
STATIC_INLINE void gc_big_object_link(bigval_t *hdr, bigval_t **list) PYJU_NOTSAFEPOINT
{
    hdr->next = *list;
    hdr->prev = list;
    if (*list)
...
STATIC_INLINE void gc_mark_sp_init(PyjuGcMarkCache_t *gc_cache, PyjuGcMarkSp_t *sp)
{
...
void gc_mark_queue_all_roots(PyjuPtls_t ptls, PyjuGcMarkSp_t *sp);
void gc_mark_queue_finlist(PyjuGcMarkCache_t *gc_cache, PyjuGcMarkSp_t *sp,
                           arraylist_t *list, size_t start);
void gc_mark_loop(PyjuPtls_t ptls, PyjuGcMarkSp_t sp);
void sweep_stack_pools(void);
void pyju_gc_debug_init(void);
...
void pyju_gc_init_page(void);
NOINLINE pyju_gc_pagemeta_t *pyju_gc_alloc_page(void) PYJU_NOTSAFEPOINT;
void pyju_gc_free_page(void *p) PYJU_NOTSAFEPOINT;
...
#ifdef GC_TIME
void gc_time_pool_start(void) PYJU_NOTSAFEPOINT;
void gc_time_count_page(int freedall, int pg_skpd) PYJU_NOTSAFEPOINT;
void gc_time_pool_end(int sweep_full) PYJU_NOTSAFEPOINT;
void gc_time_sysimg_end(uint64_t t0) PYJU_NOTSAFEPOINT;

void gc_time_big_start(void) PYJU_NOTSAFEPOINT;
void gc_time_count_big(int old_bits, int bits) PYJU_NOTSAFEPOINT;
void gc_time_big_end(void) PYJU_NOTSAFEPOINT;

void gc_time_mallocd_array_start(void) PYJU_NOTSAFEPOINT;
void gc_time_count_mallocd_array(int bits) PYJU_NOTSAFEPOINT;
void gc_time_mallocd_array_end(void) PYJU_NOTSAFEPOINT;
...
STATIC_INLINE void gc_time_count_page(int freedall, int pg_skpd) PYJU_NOTSAFEPOINT
{
    (void)freedall;
    (void)pg_skpd;
}
...
STATIC_INLINE void gc_time_count_big(int old_bits, int bits) PYJU_NOTSAFEPOINT
{
    (void)old_bits;
    (void)bits;
}
...
STATIC_INLINE void gc_time_count_mallocd_array(int bits) PYJU_NOTSAFEPOINT
{
    (void)bits;
}
...
#ifdef GC_VERIFY
extern pyju_value_t *lostval;
void gc_verify(PyjuPtls_t ptls);
void add_lostval_parent(pyju_value_t *parent);
#define verify_val(v) do {                                              \
        if (lostval == (pyju_value_t*)(v) && (v) != 0) {                  \
            pyju_printf(PYJU_STDOUT,                                        \
                      "Found lostval %p at %s:%d oftype: ",             \
                      (void*)(lostval), __FILE__, __LINE__);            \
            pyju_static_show(PYJU_STDOUT, pyju_typeof(v));                    \
            pyju_printf(PYJU_STDOUT, "\n");                                 \
        }                                                               \
    } while(0);
...
#define verify_parent(ty, obj, slot, args...) do {                      \
        if (gc_ptr_clear_tag(*(void**)(slot), 3) == (void*)lostval &&   \
            (pyju_value_t*)(obj) != lostval) {                            \
            pyju_printf(PYJU_STDOUT, "Found parent %p %p at %s:%d\n",       \
                      (void*)(ty), (void*)(obj), __FILE__, __LINE__);   \
            pyju_printf(PYJU_STDOUT, "\tloc %p : ", (void*)(slot));         \
            pyju_printf(PYJU_STDOUT, args);                                 \
            pyju_printf(PYJU_STDOUT, "\n");                                 \
            pyju_printf(PYJU_STDOUT, "\ttype: ");                           \
            pyju_static_show(PYJU_STDOUT, pyju_typeof(obj));                  \
            pyju_printf(PYJU_STDOUT, "\n");                                 \
            add_lostval_parent((pyju_value_t*)(obj));                     \
        }                                                               \
    } while(0);
...
NOINLINE void gc_mark_loop_unwind(PyjuPtls_t ptls, PyjuGcMarkSp_t sp, int pc_offset);


#ifdef GC_DEBUG_ENV
PYJU_DLLEXPORT extern pyju_gc_debug_env_t pyju_gc_debug_env;
#define gc_sweep_always_full pyju_gc_debug_env.always_full
int pyju_gc_debug_check_other(void);
int gc_debug_check_pool(void);
void pyju_gc_debug_print(void);
void gc_scrub_record_task(PyjuTask_t *ta) PYJU_NOTSAFEPOINT;
void gc_scrub(void);
#else
#define gc_sweep_always_full 0
static inline int pyju_gc_debug_check_other(void)
{
    return 0;
}
...
static inline void pyju_gc_debug_print(void)
{
}
static inline void gc_scrub_record_task(PyjuTask_t *ta) PYJU_NOTSAFEPOINT
{
    (void)ta;
}
...
#ifdef OBJPROFILE
void objprofile_count(void *ty, int old, int sz) PYJU_NOTSAFEPOINT;
void objprofile_printall(void);
void objprofile_reset(void);
#else
static inline void objprofile_count(void *ty, int old, int sz) PYJU_NOTSAFEPOINT
{
}
...
size_t pyju_array_nbytes(PyjuArray_t *a) PYJU_NOTSAFEPOINT;

PYJU_DLLEXPORT void pyju_enable_gc_logging(int enable);
void _report_gc_finished(uint64_t pause, uint64_t freed, int full, int recollect) PYJU_NOTSAFEPOINT;
