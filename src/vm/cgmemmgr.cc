...
{
    return (size > pyju_page_size * 256 ? LLT_ALIGN(size, pyju_page_size) :
            pyju_page_size * 256);
}
...
static bool check_fd_or_close(int fd)
{
    ...
    if (fchmod(fd, S_IRWXU) != 0 ||
        ftruncate(fd, pyju_page_size) != 0) {
        close(fd);
        return false;
    }
    // This can fail due to `noexec` mount option ....
    void *ptr = mmap(nullptr, pyju_page_size, PROT_READ | PROT_EXEC,
                     MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED) {
        close(fd);
        return false;
    }
    munmap(ptr, pyju_page_size);
    return true;
}
...
#  ifdef __NR_memfd_create
    fd = syscall(__NR_memfd_create, "pyju-codegen", 0);
...
#  ifdef _OS_FREEBSD_
    ...
    if (check_fd_or_close(fd))
        return fd;
#  endif
    char shm_name[PYJU_PATH_MAX] = "pyju-codegen-0123456789-0123456789/tmp///";
    pid_t pid = getpid();
    // `shm_open` can't be mapped exec on mac
#  ifndef _OS_DARWIN_
    do {
        snprintf(shm_name, sizeof(shm_name),
                 "pyju-codegen-%d-%d", (int)pid, rand());
...
static intptr_t init_shared_map()
{
    ...
    if (anon_hdl == -1)
        return -1;
    pyju_atomic_store_relaxed(&map_offset, 0);
...
{
    assert(size % pyju_page_size == 0);
    size_t off = pyju_atomic_fetch_add(&map_offset, size);
    *id = off;
...
static int _init_self_mem()
{
    ...
    void *test_pg = mmap(nullptr, pyju_page_size, PROT_READ | PROT_EXEC,
                         MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
struct Block {
    ...
    {
        if (avail >= pyju_page_size) {
            uintptr_t end = uintptr_t(ptr) + total;
            uintptr_t first_free = end - avail;
            first_free = LLT_ALIGN(first_free, pyju_page_size);
