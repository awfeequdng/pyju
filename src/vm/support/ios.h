...
PYJU_ATTRIBUTE_ALIGN_PTRSIZE(typedef struct {
...
PYJU_DLLEXPORT size_t ios_read(ios_t *s, char *dest, size_t n) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT size_t ios_readall(ios_t *s, char *dest, size_t n) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT size_t ios_write(ios_t *s, const char *data, size_t n) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT int64_t ios_seek(ios_t *s, int64_t pos) PYJU_NOTSAFEPOINT; // absolute seek
PYJU_DLLEXPORT int64_t ios_seek_end(ios_t *s) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT int64_t ios_skip(ios_t *s, int64_t offs);  // relative seek
PYJU_DLLEXPORT int64_t ios_pos(ios_t *s) PYJU_NOTSAFEPOINT;  // get current position
PYJU_DLLEXPORT int64_t ios_filesize(ios_t *s);
PYJU_DLLEXPORT int ios_trunc(ios_t *s, size_t size) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT int ios_eof(ios_t *s);
PYJU_DLLEXPORT int ios_eof_blocking(ios_t *s);
PYJU_DLLEXPORT int ios_flush(ios_t *s);
PYJU_DLLEXPORT int ios_close(ios_t *s) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT int ios_isopen(ios_t *s);
PYJU_DLLEXPORT char *ios_take_buffer(ios_t *s, size_t *psize);  // nul terminate and release buffer to caller
// set buffer space to use
PYJU_DLLEXPORT int ios_setbuf(ios_t *s, char *buf, size_t size, int own) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT int ios_bufmode(ios_t *s, bufmode_t mode) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT int ios_get_readable(ios_t *s);
PYJU_DLLEXPORT int ios_get_writable(ios_t *s);
PYJU_DLLEXPORT void ios_set_readonly(ios_t *s);
PYJU_DLLEXPORT size_t ios_copy(ios_t *to, ios_t *from, size_t nbytes);
PYJU_DLLEXPORT size_t ios_copyall(ios_t *to, ios_t *from);
PYJU_DLLEXPORT size_t ios_copyuntil(ios_t *to, ios_t *from, char delim) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT size_t ios_nchomp(ios_t *from, size_t ntowrite);
// ensure at least n bytes are buffered if possible. returns # available.
PYJU_DLLEXPORT size_t ios_readprep(ios_t *from, size_t n);
// fill the buffer and determine whether it contains the whole rest of the file
PYJU_DLLEXPORT ssize_t ios_fillbuf(ios_t *s);

/* stream creation */
PYJU_DLLEXPORT
ios_t *ios_file(ios_t *s, const char *fname, int rd, int wr, int create, int trunc) PYJU_NOTSAFEPOINT;
PYJU_DLLEXPORT ios_t *ios_mkstemp(ios_t *f, char *fname);
PYJU_DLLEXPORT ios_t *ios_mem(ios_t *s, size_t initsize) PYJU_NOTSAFEPOINT;
ios_t *ios_str(ios_t *s, char *str);
ios_t *ios_static_buffer(ios_t *s, char *buf, size_t sz);
PYJU_DLLEXPORT ios_t *ios_fd(ios_t *s, long fd, int isfile, int own);
// todo: ios_socket
extern PYJU_DLLEXPORT ios_t *ios_stdin;
extern PYJU_DLLEXPORT ios_t *ios_stdout;
extern PYJU_DLLEXPORT ios_t *ios_stderr;
void ios_init_stdstreams(void);

/* high-level functions - output */
PYJU_DLLEXPORT int ios_pututf8(ios_t *s, uint32_t wc);
PYJU_DLLEXPORT int ios_printf(ios_t *s, const char *format, ...);
PYJU_DLLEXPORT int ios_vprintf(ios_t *s, const char *format, va_list args);

/* high-level stream functions - input */
PYJU_DLLEXPORT int ios_getutf8(ios_t *s, uint32_t *pwc);
PYJU_DLLEXPORT int ios_peekutf8(ios_t *s, uint32_t *pwc);
PYJU_DLLEXPORT char *ios_readline(ios_t *s) PYJU_NOTSAFEPOINT;

// discard data buffered for reading
PYJU_DLLEXPORT void ios_purge(ios_t *s);

/* stdio-style functions */
#define IOS_EOF (-1)
PYJU_DLLEXPORT int ios_putc(int c, ios_t *s);
//wint_t ios_putwc(ios_t *s, wchar_t wc);
PYJU_DLLEXPORT int ios_getc(ios_t *s);
PYJU_DLLEXPORT int ios_peekc(ios_t *s);
