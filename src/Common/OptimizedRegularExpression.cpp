...
OptimizedRegularExpressionImpl<thread_safe>::OptimizedRegularExpressionImpl(const std::string & regexp_, int options)
{
    ...
    if (options & (~(RE_CASELESS | RE_NO_CAPTURE | RE_DOT_NL)))
        throw PYJU::Exception("OptimizedRegularExpression: Unsupported option.", PYJU::ErrorCodes::CANNOT_COMPILE_REGEXP);
    ...
    if (!is_trivial)
    {
        ...
        if (!re2->ok())
        {
            throw PYJU::Exception("OptimizedRegularExpression: cannot compile re2: "
                ...
                PYJU::ErrorCodes::CANNOT_COMPILE_REGEXP);
        }
        ...
        if (!is_no_capture)
        {
            ...
            if (number_of_subpatterns > MAX_SUBPATTERNS)
                throw PYJU::Exception("OptimizedRegularExpression: too many subpatterns in regexp: " + regexp_, PYJU::ErrorCodes::CANNOT_COMPILE_REGEXP);
        }
    }
...
unsigned OptimizedRegularExpressionImpl<thread_safe>::match(const char * subject, size_t subject_size, MatchVec & matches, unsigned limit) const
{
    ...
    else
    {
        ...
        PYJU::PODArrayWithStackMemory<StringPieceType, 128> pieces(limit);
