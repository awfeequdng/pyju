...
{
    setCodecDescription("Delta", {std::to_string(delta_bytes_size)});
}
...
{
    std::cout << "CompressionCodecDelta::updateHash: not implemented yet." << std::endl;
...
UInt8 getDeltaBytesSize(const String & data_size)
{
    size_t max_size = std::strtoul(data_size.c_str(), nullptr, 10);
...
void registerCodecDelta(CompressionCodecFactory & factory)
{
    ...
    factory.registerCompressionCodecWithType("Delta", method_code, [&](const Strings & arguments, const String & data_size) -> CompressionCodecPtr
    {
        ...
        {
            if (arguments.size() > 1)
                throw Exception("Delta codec must have 1 parameter, given " + std::to_string(arguments.size()), ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE);

            const auto literal = arguments[0];
            char *end_str;
            size_t user_bytes_size = strtoul(literal.c_str(), &end_str, 10);
            if (end_str == literal.c_str())
        ...
        {
            delta_bytes_size = getDeltaBytesSize(data_size);
        }
