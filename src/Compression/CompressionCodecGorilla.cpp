...
UInt8 getDataBytesSize(const String & data_size)
{
...
void CompressionCodecGorilla::updateHash(SipHash & hash) const
{
    ...
    std::cout << "not implemented yet: CompressionCodecGorilla::updateHash" << std::endl;
...
void registerCodecGorilla(CompressionCodecFactory & factory)
{
    ...
    factory.registerCompressionCodecWithType("Gorilla", method_code,
        [&](const Strings & arguments, const String & data_size) -> CompressionCodecPtr
    {
        if (arguments.size())
            throw Exception("Codec Gorilla does not accept any arguments", ErrorCodes::BAD_ARGUMENTS);

        UInt8 data_bytes_size = data_size.size() ? getDataBytesSize(data_size) : 0;
