...
TypeIndex typeIdx(const String &data_type)
{
    if (data_type.empty())
        return TypeIndex::Nothing;
    ...
    if (data_type == "Int8")
        ...
        return TypeIndex::UInt8;
    else if (data_type == "Enum8")
        return TypeIndex::Enum8;
    else if (data_type == "Int16")
        return TypeIndex::Int16;
    else if (data_type == "UInt16")
        return TypeIndex::UInt16;
    else if (data_type == "Enum16")
    ...
    , variant(variant_)
{
    if (variant == Variant::Byte)
        ...
        setCodecDescription("T64", {"bit"});
}

void CompressionCodecT64::updateHash(SipHash & hash) const
{
    // getCodecDesc()->updateTreeHash(hash);
    std::cout << "not implemented yet: CompressionCodecT64::updateHash" << std::endl;
...
{
    auto reg_func = [&](const Strings & arguments, const String & type) -> CompressionCodecPtr
    {
        ...
        {
            if (arguments.size() > 1)
                throw Exception("T64 support zero or one parameter, given " + std::to_string(arguments.size()),
        ...
        if (!type.empty() && type_idx == TypeIndex::Nothing)
            throw Exception("T64 codec is not supported for specified type " + type, ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE);
