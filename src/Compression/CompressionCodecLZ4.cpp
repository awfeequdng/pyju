...
{
    std::cout << "not implemented yet: CompressionCodecLZ4::updateHash" << std::endl;
...
{
    factory.registerCompressionCodec("LZ4HC", {}, [&](const Strings & arguments) -> CompressionCodecPtr
    {
        int level = 0;
        ...
        {
            if (arguments.size() > 1)
                throw Exception("LZ4HC codec must have 1 parameter, given " + std::to_string(arguments.size()), ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE);

            const auto literal = arguments[0];
            if (literal.empty())
                throw Exception("LZ4HC codec argument must be integer", ErrorCodes::ILLEGAL_CODEC_PARAMETER);
            char *end_str;
            level = strtol(literal.c_str(), &end_str, 10);
...
CompressionCodecLZ4HC::CompressionCodecLZ4HC(int level_)
    : level(level_)
{
    setCodecDescription("LZ4HC", {std::to_string(level)});
}
