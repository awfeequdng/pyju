...
{
    std::cout << "not implemented yet: CompressionCodecZSTD::updateHash" << std::endl;
...
{
    setCodecDescription(
        "ZSTD", {std::to_string(level), std::to_string(window_log)});
}

CompressionCodecZSTD::CompressionCodecZSTD(int level_) : level(level_), enable_long_range(false), window_log(0)
{
    setCodecDescription("ZSTD", {std::to_string(level)});
}

void registerCodecZSTD(CompressionCodecFactory & factory)
{
    UInt8 method_code = UInt8(CompressionMethodByte::ZSTD);
    factory.registerCompressionCodec("ZSTD", method_code, [&](const Strings & arguments) -> CompressionCodecPtr {
        int level = CompressionCodecZSTD::ZSTD_DEFAULT_LEVEL;
        if (!arguments.empty())
        {
            if (arguments.size() > 2)
                throw Exception(
                    "ZSTD codec must have 1 or 2 parameters, given " + std::to_string(arguments.size()),
                    ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE);

            const auto literal = arguments[0];
            char *end_str;
            level = std::strtol(literal.c_str(), &end_str, 10);
            ...
            if (arguments.size() > 1)
            {
                const auto window_literal = arguments[1];
                const int window_log = std::strtol(window_literal.c_str(), &end_str, 10);
                if (window_literal.empty() || end_str == window_literal.c_str())
