...
void registerEncryptionCodec(CompressionCodecFactory & factory, EncryptionMethod Method)
{
    ...
    factory.registerCompressionCodec(getMethodName(Method), method_code, [&, Method](const Strings & arguments) -> CompressionCodecPtr
    {
        if (arguments.size())
        {
...
{
    std::cout << "not implemented yet: CompressionCodecEncrypted::updateHash" << std::endl;
...
void registerEncryptionCodec(CompressionCodecFactory & factory, EncryptionMethod Method)
{
    auto throw_no_ssl = [](const Strings &) -> CompressionCodecPtr { throw Exception(ErrorCodes::OPENSSL_ERROR, "Server was built without SSL support. Encryption is disabled."); };
    const auto method_code = getMethodCode(Method); /// Codec need to know its code
