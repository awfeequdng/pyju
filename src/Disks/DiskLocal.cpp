                      ...
                      UInt64 & keep_free_space_bytes)
{
    ...
    if (name == "default")
    {
        ...
        throw Exception(
                "\"default\" disk path not supported",
                ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG);
    }
    else
    {
...
DiskLocal::DiskLocal(
    const String & name_, const String & path_, UInt64 keep_free_space_bytes_, UInt64 local_disk_check_period_ms)
    : DiskLocal(name_, path_, keep_free_space_bytes_)
{
    if (local_disk_check_period_ms > 0) {
        throw Exception("not support disk checker", ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG);
...
bool DiskLocal::setup()
{
    ...
    try
    {
        ...
        {
            auto buf = writeFile(disk_checker_path, PYJU_DEFAULT_BUFFER_SIZE, WriteMode::Rewrite);
...
void registerDiskLocal(DiskFactory & factory)
{
                      ...
                      const DisksMap & map) -> DiskPtr
    {
        String path;
        UInt64 keep_free_space_bytes;
        loadDiskLocalConfig(name, config, config_prefix, path, keep_free_space_bytes);
        ...
        std::shared_ptr<IDisk> disk
            = std::make_shared<DiskLocal>(name, path, keep_free_space_bytes, config.getUInt("local_disk_check_period_ms", 0));
        disk->startup();
