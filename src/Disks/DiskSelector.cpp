...
DiskSelector::DiskSelector(const Poco::Util::AbstractConfiguration & config, const String & config_prefix)
{
    ...
    for (const auto & disk_name : keys)
    {
        ...
        disks.emplace(disk_name, factory.create(disk_name, config, disk_config_prefix, disks));
    }
    if (!has_default_disk)
    {
        disks.emplace(
            ...
            std::make_shared<DiskLocal>(
                default_disk_name, "~", 0, config.getUInt("local_disk_check_period_ms", 0)));
    }
}


// DiskSelectorPtr DiskSelector::updateFromConfig(
//     const Poco::Util::AbstractConfiguration & config, const String & config_prefix) const
// {
//     Poco::Util::AbstractConfiguration::Keys keys;
//     config.keys(config_prefix, keys);
